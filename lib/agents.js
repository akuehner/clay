...
// Discover all non-builtin agents from the Claude CLI catalog, without the
// `chat: true` opt-in filter used by discoverChatAgents. This is the catalog
// surface for the lr-7db0 SDK-agent POC: we hand the full list to the UI and
// let the user decide which to favorite (chattable.json), then pass the
// chosen agent name through the SDK `agent` option per session.
//
