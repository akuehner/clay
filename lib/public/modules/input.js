...
export function initInput(_ctx) {
  ...
  ctx.inputEl.addEventListener("keydown", function (e) {
    ...
    if (e.key === "Enter" && !e.shiftKey && !isComposing) {
      ...
      e.preventDefault();
      // Trust the visible button mode. If the send/stop button is in stop mode
      // (processing && !hasSendableContent()), Enter must not adopt a queued
      // ghost suggestion — pressing Enter while waiting on Claude should be a
      // no-op, not a send of a stale suggestion the user never typed. (lr-e6b5)
      if (ctx.sendBtn && ctx.sendBtn.classList.contains("stop")) {
        return;
      }
  ...
  // Send/Stop button — trust the visible mode. The button's "stop" class is
  // the source of truth for what the user clicked: if the icon is the stop
  // square, the click is a Stop, never a send. Without this guard, a click
  // on the visible Stop icon could fall through the ghost-suggestion adoption
  // branch below and send a stale suggestion the user never intended to send.
  // (lr-e6b5 — regression after #337 / 0753833)
  ctx.sendBtn.addEventListener("click", function () {
    if (ctx.sendBtn.classList.contains("stop")) {
