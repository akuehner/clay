...
function renderSessionTopActions() {
  ...
  // lr-7db0 — Start agent chat. Hidden in Mate DM views per POC scope decision.
  if (!store.get('dmMode')) {
    var agentBtn = document.createElement("button");
    agentBtn.className = "session-top-action";
    agentBtn.type = "button";
    agentBtn.innerHTML = iconHtml("bot") + '<span>Start agent chat</span>';
...
function renderSessionItem(s) {
  ...
  if (s.agentName) {
    textHtml += '<span class="session-agent-badge" title="Agent: ' + escapeHtml(s.agentName) + '">'
      + iconHtml("bot") + escapeHtml(s.agentName) + '</span>';
  }
