...
function ensureStickyInfrastructure() {
  ...
  if (!stickyBottomMutObs && typeof MutationObserver !== "undefined") {
    // Watch for new direct children and observe them for size changes so
    // image loads, syntax highlighting, and tool-group expansions still
    // trigger a re-pin. Existing children at setup time are observed once.
    stickyBottomMutObs = new MutationObserver(function (mutations) {
      if (!stickyBottomResizeObs) return;
      for (var m = 0; m < mutations.length; m++) {
        var added = mutations[m].addedNodes;
        for (var n = 0; n < added.length; n++) {
          if (added[n].nodeType === 1) {
            try { stickyBottomResizeObs.observe(added[n]); } catch (e) {}
          }
        }
      }
    });
    stickyBottomMutObs.observe(messagesEl, { childList: true });
    // Seed: observe children already present.
...
function drainStreamTick() {
  ...
  if (currentFullTextLocal.length - streamRenderLen >= STREAM_RENDER_MIN_DELTA || streamBuffer.length === 0) {
    ...
    contentEl.innerHTML = renderMarkdown(currentFullTextLocal);
...
export function finalizeAssistantBlock() {
  ...
  if (msgEl) {
    ...
    if (currentFullTextLocal) {
      addCopyHandler(msgEl, currentFullTextLocal);
    }
    closeToolGroup();
  }
  currentFullTextLocal = "";
